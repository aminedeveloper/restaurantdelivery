...
/* Theme setup section
-------------------------------------------------------------------- */
if (!function_exists('pizzahouse_sc_call_to_action_theme_setup')) {
	add_action( 'pizzahouse_action_before_init_theme', 'pizzahouse_sc_call_to_action_theme_setup' );
	function pizzahouse_sc_call_to_action_theme_setup() {
		add_action('pizzahouse_action_shortcodes_list', 		'pizzahouse_sc_call_to_action_reg_shortcodes');
		if (function_exists('pizzahouse_exists_visual_composer') && pizzahouse_exists_visual_composer())
			add_action('pizzahouse_action_shortcodes_list_vc','pizzahouse_sc_call_to_action_reg_shortcodes_vc');
	}
}
...
if (!function_exists('pizzahouse_sc_call_to_action')) {
	function pizzahouse_sc_call_to_action($atts, $content=null){
		...
		if (!empty($image)) {
			$thumb_sizes = pizzahouse_get_thumb_sizes(array('layout' => 'excerpt'));
			$image = !empty($video) 
				? pizzahouse_get_resized_image_url($image, $thumb_sizes['w'], $thumb_sizes['h'])
				: pizzahouse_get_resized_image_tag($image, $thumb_sizes['w'], $thumb_sizes['h']);
		}
	
		if (!empty($video)) {
			$video = '<video' . ($id ? ' id="' . esc_attr($id.'_video') . '"' : '') 
				. ' class="sc_video"'
				. ' src="' . esc_url(pizzahouse_get_video_player_url($video)) . '"'
				...
				. '>'
				. '</video>';
			if (pizzahouse_get_custom_option('substitute_video')=='no') {
				$video = pizzahouse_get_video_frame($video, $image, '', '');
			} else {
		...
		$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '') 
				. ' class="sc_call_to_action'
					. (pizzahouse_param_is_on($accent) ? ' sc_call_to_action_accented' : '')
			...
			. '>'
				...
				. '<div class="sc_call_to_action_info'.($need_columns ? ' column-'.esc_attr($need_columns-1).'_'.esc_attr($need_columns) : '').'">'
					. (!empty($subtitle) ? '<h6 class="sc_call_to_action_subtitle sc_item_subtitle">' . trim(pizzahouse_strmacros($subtitle)) . '</h6>' : '')
					. (!empty($title) ? '<h2 class="sc_call_to_action_title sc_item_title' . (empty($description) ? ' sc_item_title_without_descr' : ' sc_item_title_without_descr') . '">' . trim(pizzahouse_strmacros($title)) . '</h2>' : '')
					. (!empty($description) ? '<div class="sc_call_to_action_descr sc_item_descr">' . trim(pizzahouse_strmacros($description)) . '</div>' : '')
					. ($style==1 ? $buttons : '')
				. '</div>'
				. ($style==2 && $align!='right' ? $buttons : '')
				. ($align=='right' ? $featured : '')
		...
		return apply_filters('pizzahouse_shortcode_output', $output, 'trx_call_to_action', $atts, $content);
	}
	add_shortcode('trx_call_to_action', 'pizzahouse_sc_call_to_action');
}
...
-------------------------------------------------------------------- */
if ( !function_exists( 'pizzahouse_sc_call_to_action_reg_shortcodes' ) ) {
	//add_action('pizzahouse_action_shortcodes_list', 'pizzahouse_sc_call_to_action_reg_shortcodes');
	function pizzahouse_sc_call_to_action_reg_shortcodes() {
	
		pizzahouse_sc_map("trx_call_to_action", array(
			"title" => esc_html__("Call to action", 'pizzahouse'),
			"desc" => wp_kses_data( __("Insert call to action block in your page (post)", 'pizzahouse') ),
			"decorate" => true,
			"container" => true,
			"params" => array(
				"title" => array(
				...
				"align" => array(
					...
					"desc" => wp_kses_data( __("Alignment elements in the block", 'pizzahouse') ),
					"value" => "",
					"type" => "checklist",
					"dir" => "horizontal",
				...
				"accent" => array(
					"title" => esc_html__("Accented", 'pizzahouse'),
					"desc" => wp_kses_data( __("Fill entire block with links color from current color scheme", 'pizzahouse') ),
					"divider" => true,
					"value" => "no",
					"type" => "switch",
				...
				"custom" => array(
					...
					"desc" => wp_kses_data( __("Allow get featured image or video from inner shortcodes (custom) or get it from shortcode parameters below", 'pizzahouse') ),
					"divider" => true,
					"value" => "no",
					"type" => "switch",
					"options" => pizzahouse_get_sc_param('yes_no')
				...
				"image" => array(
					...
					"desc" => wp_kses_data( __("Select or upload image or write URL from other site to include image into this block", 'pizzahouse') ),
					"divider" => true,
					"readonly" => false,
					"value" => "",
					"type" => "media"
				...
				"video" => array(
					...
					"desc" => wp_kses_data( __("Select video from media library or paste URL for video file from other site to include video into this block", 'pizzahouse') ),
					"readonly" => false,
					"value" => "",
					"type" => "media",
					"before" => array(
...
-------------------------------------------------------------------- */
if ( !function_exists( 'pizzahouse_sc_call_to_action_reg_shortcodes_vc' ) ) {
	//add_action('pizzahouse_action_shortcodes_list_vc', 'pizzahouse_sc_call_to_action_reg_shortcodes_vc');
	function pizzahouse_sc_call_to_action_reg_shortcodes_vc() {
	
		vc_map( array(
			"base" => "trx_call_to_action",
			"name" => esc_html__("Call to Action", 'pizzahouse'),
			"description" => wp_kses_data( __("Insert call to action block in your page (post)", 'pizzahouse') ),
			...
			"params" => array(
				array(
					"param_name" => "style",
					"heading" => esc_html__("Block's style", "pizzahouse"),
				...
				array(
					"param_name" => "accent",
					"heading" => esc_html__("Accent", "pizzahouse"),
					"description" => wp_kses_data( __("Fill entire block with links color from current color scheme", "pizzahouse") ),
					"class" => "",
				...
				array(
					...
					"description" => wp_kses_data( __("Allow get featured image or video from inner shortcodes (custom) or get it from shortcode parameters below", "pizzahouse") ),
					"class" => "",
				...
				array(
					...
					"description" => wp_kses_data( __("Image to display inside block", "pizzahouse") ),
					'dependency' => array(
						'element' => 'custom',
						'is_empty' => true
					),
				...
				array(
					...
					"description" => wp_kses_data( __("Paste URL for video file to display inside block", "pizzahouse") ),
					'dependency' => array(
						'element' => 'custom',
						'is_empty' => true
					),
...
//				),
		...
		class WPBakeryShortCode_Trx_Call_To_Action extends PIZZAHOUSE_VC_ShortCodeCollection {}
	}
}
?>