<?php
/**
 * PizzaHouse Framework: shortcodes manipulations
 *
 * @package	pizzahouse
...
// Theme init
if (!function_exists('pizzahouse_sc_theme_setup')) {
	add_action( 'pizzahouse_action_init_theme', 'pizzahouse_sc_theme_setup', 1 );
	function pizzahouse_sc_theme_setup() {
		// Add sc stylesheets
		add_action('pizzahouse_action_add_styles', 'pizzahouse_sc_add_styles', 1);
	}
}

if (!function_exists('pizzahouse_sc_theme_setup2')) {
	add_action( 'pizzahouse_action_before_init_theme', 'pizzahouse_sc_theme_setup2' );
	function pizzahouse_sc_theme_setup2() {

		if ( !is_admin() || isset($_POST['action']) ) {
			// Enable/disable shortcodes in excerpt
			add_filter('the_excerpt', 					'pizzahouse_sc_excerpt_shortcodes');
	
			// Prepare shortcodes in the content
			if (function_exists('pizzahouse_sc_prepare_content')) pizzahouse_sc_prepare_content();
		}

		// Add init script into shortcodes output in VC frontend editor
		add_filter('pizzahouse_shortcode_output', 'pizzahouse_sc_add_scripts', 10, 4);

		// AJAX: Send contact form data
		add_action('wp_ajax_send_form',			'pizzahouse_sc_form_send');
		add_action('wp_ajax_nopriv_send_form',	'pizzahouse_sc_form_send');

		// Show shortcodes list in admin editor
		add_action('media_buttons',				'pizzahouse_sc_selector_add_in_toolbar', 11);
...
if ( !function_exists( 'pizzahouse_sc_add_styles' ) ) {
	//add_action('pizzahouse_action_add_styles', 'pizzahouse_sc_add_styles', 1);
	function pizzahouse_sc_add_styles() {
		// Shortcodes
        wp_enqueue_style( 'pizzahouse-shortcodes-style',	trx_utils_get_file_url('shortcodes/theme.shortcodes.css'), array(), null );
	}
}


// Register shortcodes init scripts
if ( !function_exists( 'pizzahouse_sc_add_scripts' ) ) {
	//add_filter('pizzahouse_shortcode_output', 'pizzahouse_sc_add_scripts', 10, 4);
	function pizzahouse_sc_add_scripts($output, $tag='', $atts=array(), $content='') {

		if (pizzahouse_storage_empty('shortcodes_scripts_added')) {
			pizzahouse_storage_set('shortcodes_scripts_added', true);
			wp_enqueue_script( 'pizzahouse-shortcodes-script', trx_utils_get_file_url('shortcodes/theme.shortcodes.js'), array('jquery'), null, true );
		}
...
if (!function_exists('pizzahouse_sc_prepare_content')) {
	function pizzahouse_sc_prepare_content() {
		if (function_exists('pizzahouse_sc_clear_around')) {
			$filters = array(
				array('pizzahouse', 'sc', 'clear', 'around'),
				array('widget', 'text'),
				array('the', 'excerpt'),
				array('the', 'content')
			);
			...
			if (is_array($filters) && count($filters) > 0) {
				foreach ($filters as $flt)
					add_filter(join('_', $flt), 'pizzahouse_sc_clear_around', 1);	// Priority 1 to clear spaces before do_shortcodes()
			}
		}
	}
}
...
if (!function_exists('pizzahouse_sc_excerpt_shortcodes')) {
	//add_filter('the_excerpt', 'pizzahouse_sc_excerpt_shortcodes');
	function pizzahouse_sc_excerpt_shortcodes($content) {
		if (!empty($content)) {
...
*/
if (!function_exists('pizzahouse_sc_clear_around')) {
	function pizzahouse_sc_clear_around($content) {
...
// PizzaHouse shortcodes load scripts
if (!function_exists('pizzahouse_sc_load_scripts')) {
	function pizzahouse_sc_load_scripts() {
		static $loaded = false;
		if (!$loaded) {
			wp_enqueue_script( 'pizzahouse-shortcodes_admin-script', trx_utils_get_file_url('shortcodes/shortcodes_admin.js'), array('jquery'), null, true );
			wp_enqueue_script( 'pizzahouse-selection-script',  pizzahouse_get_file_url('js/jquery.selection.js'), array('jquery'), null, true );
			wp_localize_script( 'pizzahouse-shortcodes_admin-script', 'PIZZAHOUSE_SHORTCODES_DATA', pizzahouse_storage_get('shortcodes') );
			$loaded = true;
		}
	}
}

// PizzaHouse shortcodes prepare scripts
if (!function_exists('pizzahouse_sc_prepare_scripts')) {
	function pizzahouse_sc_prepare_scripts() {
		static $prepared = false;
		if (!$prepared) {
			pizzahouse_storage_set_array('js_vars', 'shortcodes_cp', is_admin() ? (!pizzahouse_storage_empty('to_colorpicker') ? pizzahouse_storage_get('to_colorpicker') : 'wp') : 'custom');	// wp | tiny | custom
			$prepared = true;
		}
	}
}
...
// Show shortcodes list in admin editor
if (!function_exists('pizzahouse_sc_selector_add_in_toolbar')) {
	//add_action('media_buttons','pizzahouse_sc_selector_add_in_toolbar', 11);
	function pizzahouse_sc_selector_add_in_toolbar(){

		if ( !pizzahouse_options_is_used() ) return;

		pizzahouse_sc_load_scripts();
		pizzahouse_sc_prepare_scripts();

		$shortcodes = pizzahouse_storage_get('shortcodes');
		$shortcodes_list = '<select class="sc_selector"><option value="">&nbsp;'.esc_html__('- Select Shortcode -', 'pizzahouse').'&nbsp;</option>';
		...
		pizzahouse_show_layout($shortcodes_list);
	}
}

// PizzaHouse shortcodes builder settings
...
// PizzaHouse shortcodes implementation
...
require_once trx_utils_get_file_dir('shortcodes/support.menuitems.php');
?>