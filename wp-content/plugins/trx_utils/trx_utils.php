...
if(!function_exists('trx_utils_filter_options')){
    add_filter('pizzahouse_filter_options', 'trx_utils_filter_options');
    function trx_utils_filter_options($options){
        global $PIZZAHOUSE_STORAGE;
        $custom_options = array(

            'info_other_3' => array(
                "title" => esc_html__('Additional CSS and HTML/JS code', 'pizzahouse'),
                "desc" => wp_kses_data( __('Put here your custom CSS and JS code', 'pizzahouse') ),
                "override" => "category,services_group,post,page,custom",
                "type" => "info"
            ),

            'custom_css_html' => array(
                "title" => esc_html__('Use custom CSS/HTML/JS', 'pizzahouse'),
                "desc" => wp_kses_data( __('Do you want use custom HTML/CSS/JS code in your site? For example: custom styles, Google Analitics code, etc.', 'pizzahouse') ),
                "std" => "no",
            ...
            "gtm_code" => array(
                "title" => esc_html__('Google tags manager or Google analitics code',  'pizzahouse'),
                "desc" => wp_kses_data( __('Put here Google Tags Manager (GTM) code from your account: Google analitics, remarketing, etc. This code will be placed after open body tag.',  'pizzahouse') ),
                "dependency" => array(
            ...
            "gtm_code2" => array(
                "title" => esc_html__('Google remarketing code',  'pizzahouse'),
                "desc" => wp_kses_data( __('Put here Google Remarketing code from your account. This code will be placed before close body tag.',  'pizzahouse') ),
                "dependency" => array(
                    'custom_css_html' => array('yes')
                ),
                "divider" => false,
            ...
            'custom_code' => array(
                "title" => esc_html__('Your custom HTML/JS code',  'pizzahouse'),
                "desc" => wp_kses_data( __('Put here your invisible html/js code: Google analitics, counters, etc',  'pizzahouse') ),
                "override" => "category,services_group,post,page,custom",
                "dependency" => array(
                    'custom_css_html' => array('yes')
                ),
            ...
            'custom_css' => array(
                "title" => esc_html__('Your custom CSS code',  'pizzahouse'),
                "desc" => wp_kses_data( __('Put here your css code to correct main theme styles',  'pizzahouse') ),
                "override" => "category,services_group,post,page,custom",
                "dependency" => array(
                    'custom_css_html' => array('yes')
                ),
...
if (!function_exists('pizzahouse_add_fields_in_user_info')) {
    function pizzahouse_add_fields_in_user_info( $user ) {
        ?>
        <h3><?php esc_html_e('User Position', 'pizzahouse'); ?></h3>
        <table class="form-table">
            <tr>
                <th><label for="user_position"><?php esc_html_e('User position', 'pizzahouse'); ?>:</label></th>
                <td><input type="text" name="user_position" id="user_position" size="55" value="<?php echo esc_attr(get_the_author_meta('user_position', $user->ID)); ?>" />
                    <span class="description"><?php esc_html_e('Please, enter your position in the company', 'pizzahouse'); ?></span>
                </td>
            </tr>
        </table>

        <h3><?php esc_html_e('Social links', 'pizzahouse'); ?></h3>
        <table class="form-table">
            <?php
            ...
            if (is_array($social_list) && count($social_list) > 0) {
                foreach ($social_list as $soc) {
                    ...
                    if (!empty($sn)) {
                        ?>
                        <tr>
                            <th><label for="user_<?php echo esc_attr($sn); ?>"><?php pizzahouse_show_layout(pizzahouse_strtoproper($sn)); ?>:</label></th>
                            <td><input type="text" name="user_<?php echo esc_attr($sn); ?>" id="user_<?php echo esc_attr($sn); ?>" size="55" value="<?php echo esc_attr(get_the_author_meta('user_'.($sn), $user->ID)); ?>" />
                                <span class="description"><?php echo sprintf(esc_html__('Please, enter your %s link', 'pizzahouse'), pizzahouse_strtoproper($sn)); ?></span>
                            </td>
                        </tr>
                        <?php
                    }
...
if (!function_exists('pizzahouse_save_fields_in_user_info')) {
    function pizzahouse_save_fields_in_user_info( $user_id ) {
...
if (!function_exists('pizzahouse_footer_add_scroll_to_top')) {
    //Handler of add_action('wp_footer', 'pizzahouse_footer_add_scroll_to_top');
    function pizzahouse_footer_add_scroll_to_top() {
        ?><a href="#" class="scroll_to_top icon-right-arrow" title="<?php esc_attr_e('Scroll to top', 'pizzahouse'); ?>"><?php esc_attr_e('TO TOP', 'pizzahouse'); ?></a><?php
    }
}
