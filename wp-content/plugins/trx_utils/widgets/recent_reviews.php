...
// Theme init
if (!function_exists('pizzahouse_widget_recent_reviews_theme_setup')) {
	add_action( 'pizzahouse_action_before_init_theme', 'pizzahouse_widget_recent_reviews_theme_setup', 1 );
	function pizzahouse_widget_recent_reviews_theme_setup() {

		// Register shortcodes in the shortcodes list
		if (function_exists('pizzahouse_exists_visual_composer') && pizzahouse_exists_visual_composer())
			add_action('pizzahouse_action_shortcodes_list_vc','pizzahouse_widget_recent_reviews_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('pizzahouse_widget_recent_reviews_load')) {
	add_action( 'widgets_init', 'pizzahouse_widget_recent_reviews_load' );
	function pizzahouse_widget_recent_reviews_load() {
		register_widget('pizzahouse_widget_recent_reviews');
	}
}

// Widget Class
class pizzahouse_widget_recent_reviews extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_recent_reviews', 'description' => esc_html__('Recent reviews', 'pizzahouse'));
		parent::__construct( 'pizzahouse_widget_recent_reviews', esc_html__('PizzaHouse - Recent reviews', 'pizzahouse'), $widget_ops );
...
*/
if ( !function_exists( 'pizzahouse_sc_widget_recent_reviews' ) ) {
	function pizzahouse_sc_widget_recent_reviews($atts, $content=null){	
		...
		extract($atts);
		$type = 'pizzahouse_widget_recent_reviews';
		$output = '';
		global $wp_widget_factory;
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_widget_recent_reviews' 
								. (pizzahouse_exists_visual_composer() ? ' vc_widget_recent_posts wpb_content_element' : '') 
								. (!empty($class) ? ' ' . esc_attr($class) : '') 
						. '">';
			ob_start();
			the_widget( $type, $atts, pizzahouse_prepare_widgets_args(pizzahouse_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_recent_reviews', 'widget_recent_reviews') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('pizzahouse_shortcode_output', $output, 'trx_widget_recent_reviews', $atts, $content);
	}
	add_shortcode("trx_widget_recent_reviews", "pizzahouse_sc_widget_recent_reviews");
}


// Add [trx_widget_recent_reviews] in the VC shortcodes list
if (!function_exists('pizzahouse_widget_recent_reviews_reg_shortcodes_vc')) {
	function pizzahouse_widget_recent_reviews_reg_shortcodes_vc() {
		...
		vc_map( array(
				...
				"name" => esc_html__("Widget Recent Reviews", 'pizzahouse'),
				"description" => wp_kses_data( __("Insert recent reviews list with thumbs and post's meta", 'pizzahouse') ),
