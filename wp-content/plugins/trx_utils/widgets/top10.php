...
// Theme init
if (!function_exists('pizzahouse_widget_top10_theme_setup')) {
	add_action( 'pizzahouse_action_before_init_theme', 'pizzahouse_widget_top10_theme_setup', 1 );
	function pizzahouse_widget_top10_theme_setup() {

		// Register shortcodes in the shortcodes list
		if (function_exists('pizzahouse_exists_visual_composer') && pizzahouse_exists_visual_composer())
			add_action('pizzahouse_action_shortcodes_list_vc','pizzahouse_widget_top10_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('pizzahouse_widget_top10_load')) {
	add_action( 'widgets_init', 'pizzahouse_widget_top10_load' );
	function pizzahouse_widget_top10_load() {
		register_widget('pizzahouse_widget_top10');
	}
}

// Widget Class
class pizzahouse_widget_top10 extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_top10', 'description' => esc_html__('Top 10 posts by average reviews marks (by author and users)', 'pizzahouse'));
		parent::__construct( 'pizzahouse_widget_top10', esc_html__('PizzaHouse - Top 10 Posts', 'pizzahouse'), $widget_ops );
	...
	function widget($args, $instance) {
		...
		for ($i=0; $i<2; $i++) {
			...
			$post_rating = pizzahouse_storage_get('options_prefix').'_reviews_avg'.($i==0 ? '' : '2');
		...
		if ( !empty($titles) ) {
			...
			?><div id="<?php echo esc_attr($id); ?>" class="widget_top10_content sc_tabs sc_tabs_style_2 no_jquery_ui">
				<ul class="widget_top10_tab_titles sc_tabs_titles"><?php pizzahouse_show_layout($titles); ?></ul><?php
				pizzahouse_show_layout($content);
			?></div><?php
	...
	function form($instance) {
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title_author')); ?>"><?php esc_html_e('Author rating tab title:', 'pizzahouse'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title_users')); ?>"><?php esc_html_e('Users rating tab title:', 'pizzahouse'); ?></label>
		...
		<p>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_counters')); ?>_2"><?php esc_html_e('As stars', 'pizzahouse'); ?></label>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_counters')); ?>_1"><?php esc_html_e('As icon', 'pizzahouse'); ?></label>
...
*/
if ( !function_exists( 'pizzahouse_sc_widget_top10' ) ) {
	function pizzahouse_sc_widget_top10($atts, $content=null){	
		...
		extract($atts);
		$type = 'pizzahouse_widget_top10';
		$output = '';
		global $wp_widget_factory;
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_widget_top10' 
								. (pizzahouse_exists_visual_composer() ? ' vc_widget_top10 wpb_content_element' : '') 
								. (!empty($class) ? ' ' . esc_attr($class) : '') 
						. '">';
			ob_start();
			the_widget( $type, $atts, pizzahouse_prepare_widgets_args(pizzahouse_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_top10', 'widget_top10') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('pizzahouse_shortcode_output', $output, 'trx_widget_top10', $atts, $content);
	}
	add_shortcode("trx_widget_top10", "pizzahouse_sc_widget_top10");
}


// Add [trx_widget_top10] in the VC shortcodes list
if (!function_exists('pizzahouse_widget_top10_reg_shortcodes_vc')) {
	//add_action('pizzahouse_action_shortcodes_list_vc','pizzahouse_widget_top10_reg_shortcodes_vc');
	function pizzahouse_widget_top10_reg_shortcodes_vc() {
		...
		vc_map( array(
				...
				"name" => esc_html__("Widget Top10 Reviews", 'pizzahouse'),
				"description" => wp_kses_data( __("Insert Top10 reviews list with thumbs and post's meta", 'pizzahouse') ),
				...
				"params" => array(
					...
					array(
						...
						"heading" => esc_html__("Author's tab title", 'pizzahouse'),
						"description" => wp_kses_data( __("Title for the tab with authors's reviews", 'pizzahouse') ),
						"class" => "",
						"value" => "",
						"type" => "textfield"
					),
					array(
						...
						"heading" => esc_html__("User's tab title", 'pizzahouse'),
						"description" => wp_kses_data( __("Title for the tab with authors's reviews", 'pizzahouse') ),
						"class" => "",
						"value" => "",
						"type" => "textfield"
					...
					array(
						...
						"value" => array_flip(array(
							'2' => esc_html__('As stars', 'pizzahouse'),
							'1' => esc_html__('As text', 'pizzahouse'),
							'0' => esc_html__('Hide', 'pizzahouse')
						)),
						"type" => "dropdown"
					),
