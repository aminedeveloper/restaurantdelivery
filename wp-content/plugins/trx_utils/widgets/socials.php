...
// Theme init
if (!function_exists('pizzahouse_widget_socials_theme_setup')) {
	add_action( 'pizzahouse_action_before_init_theme', 'pizzahouse_widget_socials_theme_setup', 1 );
	function pizzahouse_widget_socials_theme_setup() {

		// Register shortcodes in the shortcodes list
		if (function_exists('pizzahouse_exists_visual_composer') && pizzahouse_exists_visual_composer())
			add_action('pizzahouse_action_shortcodes_list_vc','pizzahouse_widget_socials_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('pizzahouse_widget_socials_load')) {
	add_action( 'widgets_init', 'pizzahouse_widget_socials_load' );
	function pizzahouse_widget_socials_load() {
		register_widget( 'pizzahouse_widget_socials' );
	}
}

// Widget Class
class pizzahouse_widget_socials extends WP_Widget {

	function __construct() {
		$widget_ops = array( 'classname' => 'widget_socials', 'description' => esc_html__('Show site logo and social links', 'pizzahouse') );
		parent::__construct( 'pizzahouse_widget_socials', esc_html__('PizzaHouse - Show logo and social links', 'pizzahouse'), $widget_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		$text = isset($instance['text']) ? pizzahouse_do_shortcode($instance['text']) : '';
            ...
            <?php
				if ($show_logo) {
					...
					if ($logo_image || $logo_text)
						pizzahouse_show_logo($logo_image, false, false, false, $logo_text, $logo_slogan);
				}
				...
				if ($show_icons) {
					pizzahouse_show_layout(pizzahouse_sc_socials(array('size' => "tiny", 'shape' => 'round')));
				}
			?>
		</div>
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'text' )); ?>"><?php esc_html_e('Description:', 'pizzahouse'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'logo_image' )); ?>"><?php echo wp_kses_data( __('Logo image:<br />(if empty - use logo from Theme Options)', 'pizzahouse') ); ?></label>
            ...
            <?php
			pizzahouse_show_layout(pizzahouse_show_custom_field($this->get_field_id( 'logo_media' ), array('type'=>'mediamanager', 'media_field_id'=>$this->get_field_id( 'logo_image' )), null));
			if ($logo_image) {
			?>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'logo_text' )); ?>"><?php esc_html_e('Logo text:', 'pizzahouse'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'logo_slogan' )); ?>"><?php esc_html_e('Logo slogan:', 'pizzahouse'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('show_logo')); ?>_1"><?php esc_html_e('Show logo:', 'pizzahouse'); ?></label><br />
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_logo')); ?>_1"><?php esc_html_e('Show', 'pizzahouse'); ?></label>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_logo')); ?>_0"><?php esc_html_e('Hide', 'pizzahouse'); ?></label>
		</p>
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('show_icons')); ?>_1"><?php esc_html_e('Show social icons:', 'pizzahouse'); ?></label><br />
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_icons')); ?>_1"><?php esc_html_e('Show', 'pizzahouse'); ?></label>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_icons')); ?>_0"><?php esc_html_e('Hide', 'pizzahouse'); ?></label>
		</p>
...
*/
if ( !function_exists( 'pizzahouse_sc_widget_socials' ) ) {
	function pizzahouse_sc_widget_socials($atts, $content=null){	
		...
		extract($atts);
		$type = 'pizzahouse_widget_socials';
		$output = '';
		global $wp_widget_factory;
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
							. ' class="widget_area sc_widget_socials' 
								. (pizzahouse_exists_visual_composer() ? ' vc_widget_socials wpb_content_element' : '') 
								. (!empty($class) ? ' ' . esc_attr($class) : '') 
						. '">';
			ob_start();
			the_widget( $type, $atts, pizzahouse_prepare_widgets_args(pizzahouse_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_socials', 'widget_socials') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('pizzahouse_shortcode_output', $output, 'trx_widget_socials', $atts, $content);
	}
	add_shortcode("trx_widget_socials", "pizzahouse_sc_widget_socials");
}


// Add [trx_widget_socials] in the VC shortcodes list
if (!function_exists('pizzahouse_widget_socials_reg_shortcodes_vc')) {
	function pizzahouse_widget_socials_reg_shortcodes_vc() {
		
		vc_map( array(
				"base" => "trx_widget_socials",
				"name" => esc_html__("Widget Socials", 'pizzahouse'),
				"description" => wp_kses_data( __("Insert site logo, description and/or socials list", 'pizzahouse') ),
				...
				"params" => array(
					...
					array(
						"param_name" => "text",
						"heading" => esc_html__("Widget text", 'pizzahouse'),
						"description" => wp_kses_data( __("Any description", 'pizzahouse') ),
						"class" => "",
						"value" => "",
						"type" => "textarea"
					),
					array(
						...
						"heading" => esc_html__("Show logo", 'pizzahouse'),
						"description" => wp_kses_data( __("Do you want display logo image?", 'pizzahouse') ),
						"class" => "",
						"std" => 1,
					...
					array(
						...
						"heading" => esc_html__("Show social icons", 'pizzahouse'),
						"description" => wp_kses_data( __("Do you want display social icons?", 'pizzahouse') ),
						"class" => "",
						"std" => 1,
					...
					array(
						...
						"heading" => esc_html__("Logo image", 'pizzahouse'),
						"description" => wp_kses_data( __("Select or upload image or write URL from other site for the logo (leave empty if you want use default site logo)", 'pizzahouse') ),
						'dependency' => array(
							'element' => 'show_logo',
							'not_empty' => true
						),
					...
					array(
						...
						"heading" => esc_html__("Logo text", 'pizzahouse'),
						"description" => wp_kses_data( __("Site name for the logo(leave empty if you want use default site text)", 'pizzahouse') ),
						"admin_label" => true,
						'dependency' => array(
							'element' => 'show_logo',
							'not_empty' => true
					...
					array(
						...
						"heading" => esc_html__("Logo slogan", 'pizzahouse'),
						"description" => wp_kses_data( __("Site slogan for the logo(leave empty if you want use default site tagline)", 'pizzahouse') ),
						'dependency' => array(
							'element' => 'show_logo',
							'not_empty' => true
						),
