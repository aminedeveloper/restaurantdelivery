...
// Theme init
if (!function_exists('pizzahouse_widget_recent_posts_theme_setup')) {
	add_action( 'pizzahouse_action_before_init_theme', 'pizzahouse_widget_recent_posts_theme_setup', 1 );
	function pizzahouse_widget_recent_posts_theme_setup() {

		// Register shortcodes in the shortcodes list
		if (function_exists('pizzahouse_exists_visual_composer') && pizzahouse_exists_visual_composer())
			add_action('pizzahouse_action_shortcodes_list_vc','pizzahouse_widget_recent_posts_reg_shortcodes_vc');
	}
}

// Load widget
if (!function_exists('pizzahouse_widget_recent_posts_load')) {
	add_action( 'widgets_init', 'pizzahouse_widget_recent_posts_load' );
	function pizzahouse_widget_recent_posts_load() {
		register_widget('pizzahouse_widget_recent_posts');
	}
}

// Widget Class
class pizzahouse_widget_recent_posts extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget_recent_posts', 'description' => esc_html__('The recent blog posts (extended)', 'pizzahouse'));
		parent::__construct( 'pizzahouse_widget_recent_posts', esc_html__('PizzaHouse - Recent Posts', 'pizzahouse'), $widget_ops );
	...
	function form($instance) {
		...
		$counters = array_merge(array('hide' => esc_html__('Hide', 'pizzahouse')), pizzahouse_get_list_blog_counters(false));
		?>
...
*/
if ( !function_exists( 'pizzahouse_sc_widget_recent_posts' ) ) {
	function pizzahouse_sc_widget_recent_posts($atts, $content=null){	
		...
		extract($atts);
		$type = 'pizzahouse_widget_recent_posts';
		$output = '';
		...
		if ( is_object( $wp_widget_factory ) && isset( $wp_widget_factory->widgets, $wp_widget_factory->widgets[ $type ] ) ) {
			$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
						...
						. '">';
			ob_start();
			the_widget( $type, $atts, pizzahouse_prepare_widgets_args(pizzahouse_storage_get('widgets_args'), $id ? $id.'_widget' : 'widget_recent_posts', 'widget_recent_posts') );
			$output .= ob_get_contents();
			ob_end_clean();
			$output .= '</div>';
		}
		return apply_filters('pizzahouse_shortcode_output', $output, 'trx_widget_recent_posts', $atts, $content);
	}
	add_shortcode("trx_widget_recent_posts", "pizzahouse_sc_widget_recent_posts");
}


// Add [trx_widget_recent_posts] in the VC shortcodes list
if (!function_exists('pizzahouse_widget_recent_posts_reg_shortcodes_vc')) {
	function pizzahouse_widget_recent_posts_reg_shortcodes_vc() {
		...
		vc_map( array(
				...
				"name" => esc_html__("Widget Recent Posts", 'pizzahouse'),
				"description" => wp_kses_data( __("Insert recent posts list with thumbs and post's meta", 'pizzahouse') ),
