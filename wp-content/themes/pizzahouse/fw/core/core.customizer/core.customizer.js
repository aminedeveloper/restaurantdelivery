...
jQuery(document).ready(function(){
	...
	jQuery('.to_demo_wrap').each(function(idx) {
		"use strict";
		var tab = jQuery(this).parents('.pizzahouse_options_tab_content');
		var tab_id = tab.attr('id');
		var scheme = tab_id.substring(4, tab_id.length-8);
		var info = jQuery(this).parents('.pizzahouse_options_field_info').attr('id', 'to_demo_scheme_'+scheme).attr('data-scheme', scheme).data('scheme', scheme);
		if (idx==0 && jQuery(this).find('.to_demo_pin').length==1 && pizzahouse_get_cookie('to_demo_fixed')==1) pizzahouse_options_fix_demo(info);
	});

	// Stick demo panel
	jQuery(document).on('click', '.pizzahouse_options_field_info .to_demo_pin', function(e) {
		pizzahouse_options_fix_demo(jQuery(this).parents('.pizzahouse_options_field_info'));
		e.preventDefault();
		return false;
	});	

	// Update demo on page load
	pizzahouse_options_decorate_demo();

	// Update demo on change fields
	jQuery('#partition_schemes_content').on('change', '.pizzahouse_options_input,.pizzahouse_options_field input[type="hidden"]', function(e) {
		"use strict";
		var tab = jQuery(this).parents('.pizzahouse_options_tab_content');
		var tab_id = tab.attr('id');
		var scheme = tab_id.substring(4, tab_id.length-8);
		pizzahouse_options_decorate_demo(scheme);
	});

	// Update demo on blur color fields
	jQuery('#partition_schemes_content').on('blur', '.pizzahouse_options_input_color', function(e) {
		"use strict";
		var tab = jQuery(this).parents('.pizzahouse_options_tab_content');
		var tab_id = tab.attr('id');
		var scheme = tab_id.substring(4, tab_id.length-8);
	...
	jQuery('#partition_schemes_content .pizzahouse_options_tab > ul').sortable({
		update: function(event, ui) {
			var tabs = "";
			ui.item.parents('.pizzahouse_options_tab').find('> ul > li').each(function() {
				tabs += (tabs ? PIZZAHOUSE_STORAGE['to_delimiter'] : '') + jQuery(this).attr('id').substr(4);
			});
			ui.item.parents('.pizzahouse_options_tab').find('> .pizzahouse_options_schemes_order').val(tabs);
		}
	}).disableSelection().after('<input type="hidden" name="pizzahouse_options_schemes_order" class="pizzahouse_options_schemes_order" value="">');
});


// When switch tabs - update fixed demo block
function pizzahouse_init_hidden_elements(panel) {
	"use strict";
	var demo = panel.find('.pizzahouse_options_field_info[data-scheme]');
	if (demo.length == 0) return;
	var info2 = jQuery('.pizzahouse_options').find('>.pizzahouse_options_field_info');
...
function pizzahouse_options_action_scheme_delete(obj) {
	"use strict";
	var button = jQuery(obj);
	var tab_id = button.parents('.pizzahouse_options_tab_content').attr('id');
	var scheme = tab_id.substring(4, tab_id.length-8);
	pizzahouse_message_confirm(PIZZAHOUSE_STORAGE['to_strings']['scheme_delete_confirm'], PIZZAHOUSE_STORAGE['to_strings']['scheme_delete']+': "'+scheme+'"', function(btn) {
		"use strict";
		if (btn != 1) return;
		var data = {
			action: 'pizzahouse_options_scheme_delete',
			nonce: PIZZAHOUSE_STORAGE['ajax_nonce'],
			scheme: scheme,
			order: button.parents('.pizzahouse_options_tab').find('> .pizzahouse_options_schemes_order').val()
		};
		setTimeout(function(){
			pizzahouse_message_info(PIZZAHOUSE_STORAGE['to_strings']['recompile_styles'], PIZZAHOUSE_STORAGE['to_strings']['wait'], 'spin3 animate-spin', 60000);
		}, 600);
		jQuery.post(PIZZAHOUSE_STORAGE['ajax_url'], data, function(response) {
			...
			if (rez.error === '') {
				pizzahouse_message_success(PIZZAHOUSE_STORAGE['to_strings']['scheme_delete_complete']+'<br>'+PIZZAHOUSE_STORAGE['to_strings']['reload_page'], PIZZAHOUSE_STORAGE['to_strings']['scheme_delete']);
				setTimeout(function() { location.reload(); }, 3000);
			} else {
				pizzahouse_message_warning(PIZZAHOUSE_STORAGE['to_strings']['scheme_delete_failed']+':<br>'+rez.error, PIZZAHOUSE_STORAGE['to_strings']['scheme_delete']);
			}
		});
...
function pizzahouse_options_action_scheme_copy(obj) {
	"use strict";
	var button = jQuery(obj);
	var tab_id = button.parents('.pizzahouse_options_tab_content').attr('id');
	var scheme = tab_id.substring(4, tab_id.length-8);
	pizzahouse_message_confirm(PIZZAHOUSE_STORAGE['to_strings']['scheme_copy_confirm'], PIZZAHOUSE_STORAGE['to_strings']['scheme_copy']+': "'+scheme+'"', function(btn) {
		"use strict";
		if (btn != 1) return;
		var data = {
			action: 'pizzahouse_options_scheme_copy',
			nonce: PIZZAHOUSE_STORAGE['ajax_nonce'],
			scheme: scheme,
			order: button.parents('.pizzahouse_options_tab').find('> .pizzahouse_options_schemes_order').val()
		};
		...
		jQuery.post(PIZZAHOUSE_STORAGE['ajax_url'], data, function(response) {
			...
			if (rez.error === '') {
				pizzahouse_message_success(PIZZAHOUSE_STORAGE['to_strings']['scheme_copy_complete']+'<br>'+PIZZAHOUSE_STORAGE['to_strings']['reload_page'], PIZZAHOUSE_STORAGE['to_strings']['scheme_copy']);
				setTimeout(function() { location.reload(); }, 3000);
			} else {
				pizzahouse_message_warning(PIZZAHOUSE_STORAGE['to_strings']['scheme_copy_failed']+':<br>'+rez.error, PIZZAHOUSE_STORAGE['to_strings']['scheme_copy']);
			}
		});
...
function pizzahouse_options_fix_demo(info) {
	var info2 = info.parents('.pizzahouse_options').toggleClass('to_demo_fixed').find('>.pizzahouse_options_field_info');
	if (info2.length == 0) {
		var clonedObj = info.clone();
		clonedObj.removeAttr('id');
		info.parents('.pizzahouse_options').append(clonedObj);
		info2 = info.parents('.pizzahouse_options').find('>.pizzahouse_options_field_info');
	}
	pizzahouse_set_cookie('to_demo_fixed', jQuery('.pizzahouse_options').hasClass('to_demo_fixed') ? 1 : 0, 365);
}


// Decorate demo block
function pizzahouse_options_decorate_demo() {
	"use strict";
	...
	jQuery('.to_demo_body').each(function() {
		...
		var scheme = jQuery(this).parents('.pizzahouse_options_field_info').data('scheme');
