/* global pizzahouse_color_schemes, pizzahouse_dependencies, Color */
...
( function( api ) {
	...
	if (!pizzahouse_customizer_vars['need_refresh']) {
		for (var i in pizzahouse_color_schemes) {
			cssTemplate[i] = wp.template( 'pizzahouse-color-scheme-'+i );
		}
		cssTemplate['fonts'] = wp.template( 'pizzahouse-fonts' );
	}
	...
	api.bind('ready', function() {
		...
		// Add 'Refresh' button
		if (pizzahouse_customizer_vars['need_refresh']) {
			jQuery('#customize-header-actions .spinner').after('<input type="button" class="button customize-action-refresh" value="'+pizzahouse_customizer_vars['msg_refresh']+'">');
		...
		jQuery('#customize-theme-controls .control-section').each(function () {
			pizzahouse_customizer_wp_check_dependencies(jQuery(this));
		});

		// Update colors
		pizzahouse_customizer_wp_change_color_scheme(api('color_scheme')());
	});
	
	// On change any control - check for dependencies
	api.bind('change', function(obj) {
		if (obj.id == 'scheme_storage') return;
		pizzahouse_customizer_wp_check_dependencies(jQuery('#customize-theme-controls #customize-control-'+obj.id).parents('.control-section'));
		pizzahouse_customizer_wp_refresh_preview(obj);
	});

	// Check for dependencies
	function pizzahouse_customizer_wp_check_dependencies(cont) {
		"use strict";
		...
		cont.find('.customize-control').each(function() {
			...
			var depend = false;
			for (var fld in pizzahouse_dependencies) {
				if (fld == id) {
					depend = pizzahouse_dependencies[id];
					break;
				}
			}
			if (depend) {
				...
				if ((dep_cnt > 0 && dep_cmp == 'or') || (dep_cnt == dep_all && dep_cmp == 'and')) {
					jQuery(this).show().removeClass('pizzahouse_options_no_use');
				} else {
					jQuery(this).hide().addClass('pizzahouse_options_no_use');
				}
			}
		});
	}
	...
	function pizzahouse_customizer_wp_refresh_preview(obj) {
		"use strict";
		...
		if (id == 'color_scheme') {
			pizzahouse_customizer_wp_change_color_scheme(val);
		} else if (updateCSS) {
			var simple = api('color_settings')()=='simple';
			for (var opt in pizzahouse_color_schemes['original']) {
				if (opt == id) {
					updateCSS = false;
					// Store new value in the color table
					pizzahouse_customizer_wp_update_color_scheme(opt, val);
					...
					if (simple) {
						if (id == 'text_link') {
							...
							pizzahouse_customizer_wp_update_color_scheme('alter_link', val);
						} else if (id == 'text_hover') {
							api('alter_hover').set( val );
							api.control( 'alter_hover' ).container.find( '.color-picker-hex' )
								.data( 'data-default-color', val )
								.wpColorPicker( 'defaultColor', val );
							pizzahouse_customizer_wp_update_color_scheme('alter_hover', val);
						}
					}
					updateCSS = true;
					processed = true;
			...
			if (!processed) {
				for (var tag in pizzahouse_fonts) {
					for (var prop in pizzahouse_fonts[tag]) {
						if (prop=='title' || prop=='description') continue;
						if (tag+'-'+prop == id) {
							processed = true;
							// Store new value in the fonts table
							pizzahouse_customizer_wp_update_fonts(tag, prop, val);
							break;
						}
					}
					if (processed) break;
			...
			// Refresh CSS
			if (processed) pizzahouse_customizer_wp_update_css();
		}
	...
	// Store new value in the color table
	function pizzahouse_customizer_wp_update_color_scheme(opt, value) {
		"use strict";
		pizzahouse_color_schemes[api('color_scheme')()][opt] = value;
		api('scheme_storage').set(pizzahouse_serialize(pizzahouse_color_schemes))
	}
	

	// Change color scheme - update colors and generate css
	function pizzahouse_customizer_wp_change_color_scheme(value) {
		"use strict";
		updateCSS = false;
		for (var opt in pizzahouse_color_schemes[value]) {
			if (api(opt) == undefined) continue;
			api( opt ).set( pizzahouse_color_schemes[value][opt] );
			api.control( opt ).container.find( '.color-picker-hex' )
				.data( 'data-default-color', pizzahouse_color_schemes[value][opt] )
				.wpColorPicker( 'defaultColor', pizzahouse_color_schemes[value][opt] );
		}
		updateCSS = true;
		pizzahouse_customizer_wp_update_css();
	}

	// Store new value in the fonts table
	function pizzahouse_customizer_wp_update_fonts(tag, prop, value) {
		"use strict";
		pizzahouse_fonts[tag][prop] = value;
	}
	
	// Generate the CSS for the current Color Scheme and send it to the preview window
	function pizzahouse_customizer_wp_update_css() {
		"use strict";

		if (!updateCSS || pizzahouse_customizer_vars['need_refresh']) return;

		var css = '';
		
		// Add color styles
		for (var scheme in pizzahouse_color_schemes) {
			
			var colors = [];
			
			// Copy all colors!
			for (var i in pizzahouse_color_schemes[scheme]) {
				if (i=='title') continue;
				colors[i] = pizzahouse_color_schemes[scheme][i];
			}
		...
		// Make theme specific colors and tints
		if (window.pizzahouse_customizer_wp_add_theme_fonts) fonts = pizzahouse_customizer_wp_add_theme_fonts(fonts);
	...
	function pizzahouse_customizer_wp_add_theme_fonts(fonts) {
		fonts['logo_margin-top_0_75']	= pizzahouse_multiply_css_value( fonts['logo_margin-top_value'], 0.75 );
		fonts['logo_margin-bottom_0_5']	= pizzahouse_multiply_css_value( fonts['logo_margin-bottom_value'], 0.5 );
		fonts['menu_margin-top_1']		= fonts['menu_margin-top_value'];
		fonts['menu_margin-top_0_3']	= pizzahouse_multiply_css_value( fonts['menu_margin-top_value'], 0.3 );
		fonts['menu_margin-top_0_6']	= pizzahouse_multiply_css_value( fonts['menu_margin-top_value'], 0.6 );
		fonts['menu_margin-top_0_65']	= pizzahouse_multiply_css_value( fonts['menu_margin-top_value'], 0.65 );
		fonts['menu_margin-top_0_7']	= pizzahouse_multiply_css_value( fonts['menu_margin-top_value'], 0.7 );
		fonts['menu_margin-top_0_8']	= pizzahouse_multiply_css_value( fonts['menu_margin-top_value'], 0.8 );
		fonts['menu_margin-bottom_1']	= fonts['menu_margin-bottom_value'];
		fonts['menu_margin-bottom_0_5']	= pizzahouse_multiply_css_value( fonts['menu_margin-bottom_value'], 0.5 );
		fonts['menu_margin-bottom_0_6']	= pizzahouse_multiply_css_value( fonts['menu_margin-bottom_value'], 0.6 );
