...
function pizzahouse_theme_customizer(option, val) {
	"use strict";
	var custom_style = '';
	...
	if (option == 'body_style') {
		...
		var params = PIZZAHOUSE_STORAGE['co_add_params']!=undefined ? PIZZAHOUSE_STORAGE['co_add_params'] : {};
		params[option] = val;
	...
	if (custom_style != '') {
		var styles = jQuery('#pizzahouse-customizer-styles-'+option).length > 0 ? jQuery('#pizzahouse-customizer-styles-'+option) : '';
		if (styles.length == 0)
			jQuery('head').append('<style id="pizzahouse-customizer-styles-'+option+'" type="text/css">'+custom_style+'</style>');
		else
