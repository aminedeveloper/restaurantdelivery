...
if ( !function_exists( 'pizzahouse_template_services_4_theme_setup' ) ) {
	add_action( 'pizzahouse_action_before_init_theme', 'pizzahouse_template_services_4_theme_setup', 1 );
	function pizzahouse_template_services_4_theme_setup() {
		pizzahouse_add_template(array(
			'layout' => 'services-4',
			'template' => 'services-4',
			'mode'   => 'services',
			'need_columns' => true,
			'title'  => esc_html__('Services /Style 4/', 'pizzahouse')
		));
	}
}

// Template output
if ( !function_exists( 'pizzahouse_template_services_4_output' ) ) {
	function pizzahouse_template_services_4_output($post_options, $post_data) {
		...
		?>
			<div<?php echo !empty($post_options['tag_id']) ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''; ?>
				...
				pizzahouse_show_layout(pizzahouse_do_shortcode('[trx_icon icon="'.esc_attr($post_data['post_icon']).'" shape="round"]'));
				?><span class="sc_services_item_title"><?php pizzahouse_show_layout($post_data['post_title']); ?></span><?php
				if ((!isset($post_options['links']) || $post_options['links']) && !empty($post_data['post_link'])) {
					?></a><?php
				}
				?>
