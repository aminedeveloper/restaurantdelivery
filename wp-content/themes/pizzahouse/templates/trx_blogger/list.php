...
if ( !function_exists( 'pizzahouse_template_list_theme_setup' ) ) {
	add_action( 'pizzahouse_action_before_init_theme', 'pizzahouse_template_list_theme_setup', 1 );
	function pizzahouse_template_list_theme_setup() {
		pizzahouse_add_template(array(
			'layout' => 'list',
			'mode'   => 'blogger',
			'need_columns' => true,
			'title'  => esc_html__('Blogger layout: List', 'pizzahouse')
			));
	}
}

// Template output
if ( !function_exists( 'pizzahouse_template_list_output' ) ) {
	function pizzahouse_template_list_output($post_options, $post_data) {
