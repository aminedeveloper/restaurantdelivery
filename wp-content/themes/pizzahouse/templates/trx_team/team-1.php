...
if ( !function_exists( 'pizzahouse_template_team_1_theme_setup' ) ) {
	add_action( 'pizzahouse_action_before_init_theme', 'pizzahouse_template_team_1_theme_setup', 1 );
	function pizzahouse_template_team_1_theme_setup() {
		pizzahouse_add_template(array(
			'layout' => 'team-1',
			'template' => 'team-1',
			'mode'   => 'team',
			'title'  => esc_html__('Team /Style 1/', 'pizzahouse'),
...
// Template output
if ( !function_exists( 'pizzahouse_template_team_1_output' ) ) {
	function pizzahouse_template_team_1_output($post_options, $post_data) {
		$show_title = true;
		...
		?>
			<div<?php echo !empty($post_options['tag_id']) ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''; ?>
				...
				<div class="sc_team_item_avatar"><?php pizzahouse_show_layout($post_options['photo']); ?></div>
